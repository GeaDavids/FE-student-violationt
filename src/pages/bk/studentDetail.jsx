...
const StudentDetail = () => {
        ...
        html: `
          <div class="text-left space-y-3">
            <div class="border-b pb-2 mb-3">
              <p><strong>Jenis:</strong> ${getJenisSuratText(detailSurat.jenisSurat)}</p>
              <p><strong>Tingkat:</strong> Level ${detailSurat.tingkatSurat}</p>
              <p><strong>Total Score:</strong> <span class="text-red-600 font-bold">${detailSurat.totalScoreSaat}</span></p>
              <p><strong>Status:</strong> ${getStatusText(detailSurat.statusKirim)}</p>
              <p><strong>Tanggal Dibuat:</strong> ${new Date(detailSurat.createdAt).toLocaleDateString("id-ID")}</p>
            </div>
            <div>
              <h4 class="font-bold mb-2">Isi Surat:</h4>
              <div class="bg-gray-50 p-3 rounded text-sm max-h-60 overflow-y-auto whitespace-pre-line">${detailSurat.isiSurat}</div>
            </div>
          </div>
        `,
        width: "600px",
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-1.5 py-0.5 rounded-full text-xs font-semibold ${colors[status]}`}>
        {labels[status]}
      </span>
    );
  };
  ...
  const getJenisSuratBadge = (jenis) => {
    ...
    return (
      <span className={`px-1.5 py-0.5 rounded-full text-xs font-semibold ${colors[jenis]}`}>
        {labels[jenis] || jenis}
      </span>
    );
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-sm font-semibold text-slate-900 mb-1">Terjadi Kesalahan</h3>
      ...
      <div className="container mx-auto px-3 py-4 max-w-6xl">
          ...
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-sm font-semibold text-slate-900">{siswa.nisn}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-slate-900">{siswa.tempatLahir}</p>
              </div>
              <div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-md border border-slate-200 mb-4">
          <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white p-4 rounded-t-lg">
            <h2 className="text-lg font-bold">Riwayat Pelanggaran</h2>
            <p className="text-orange-100 text-xs mt-1">
              Total {laporan?.length || 0} laporan pelanggaran
            </p>
          </div>
          <div className="p-4">
            {laporan && laporan.length > 0 ? (
                ...
                <table className="min-w-full text-xs">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Tanggal</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Jenis Pelanggaran</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Poin</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Pelapor</th>
                    ...
                    {laporan.map((item, index) => (
                      <tr key={index} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-2 px-3 font-medium text-slate-900">
                          {item.jenisViolation?.namaViolation || "-"}
                        </td>
                        <td className="py-2 px-3">
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                            -{item.jenisViolation?.poin || 0}
                          </span>
                        </td>
                        <td className="py-2 px-3 text-slate-600">
              ...
              <div className="text-center py-6">
                ...
                <h3 className="text-sm font-semibold text-slate-900 mb-1">Tidak Ada Pelanggaran</h3>
                <p className="text-slate-500 text-xs">Siswa ini belum memiliki catatan pelanggaran</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="p-4">
            {loadingSurat ? (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-xs">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Tanggal</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Jenis Surat</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Total Poin</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Status</th>
                      <th className="text-left py-2 px-3 font-semibold text-slate-700">Aksi</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              <div className="text-center py-6">
                ...
                <h3 className="text-sm font-semibold text-slate-900 mb-1">Belum Ada Surat Peringatan</h3>
                <p className="text-slate-500 text-xs">Siswa ini belum pernah mendapat surat peringatan</p>
              </div>
            )}
          </div>
        </div>
