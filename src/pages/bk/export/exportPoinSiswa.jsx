...
const ExportPoinSiswa = () => {
  ...
  useEffect(() => {
    ...
    academicYearAPI.getAll().then((response) => {
      const years = response.data.data || [];
      setTahunAjaranList(years);
      
      // Auto-select current academic year
      academicYearAPI.getCurrent().then((currentResponse) => {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 p-4 mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">Export Poin Siswa</h1>
              <p className="text-gray-600 text-xs mt-0.5">Download rekap poin dalam format Excel</p>
            </div>
          </div>
        </div>
        {/* Filter Section */}
            ...
            {preview.length > 0 && (
              <div className="bg-purple-50 border border-purple-200 rounded-md px-3 py-1.5">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-purple-800 font-medium text-xs">{preview.length} siswa ditemukan</span>
                </div>
              </div>
            )}
          </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {preview.map((s, i) => (
                      <tr key={i} className="hover:bg-gray-50 transition-colors duration-150">
                        <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-900">{s.nisn}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-900">{s.nama}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-600">{s.kelas}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-600">{s.angkatan}</td>
                        <td className="px-3 py-2 whitespace-nowrap">
                          <span className={`text-xs font-semibold ${s.totalScore > 100 ? 'text-red-600' : s.totalScore > 50 ? 'text-yellow-600' : 'text-green-600'}`}>
                            {s.totalScore}
                          </span>
                        </td>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-md p-3 mt-4">
          <div className="flex items-start space-x-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-purple-900 mb-1">Informasi Export Poin Siswa</h3>
              <div className="space-y-1 text-xs text-purple-800">
                <p>• <span className="font-medium">Data Komprehensif:</span> Rekap lengkap poin pelanggaran, prestasi, dan penanganan setiap siswa</p>
                <p>• <span className="font-medium">Filter Fleksibel:</span> Kosongkan filter untuk mengexport semua data siswa</p>
                <p>• <span className="font-medium">Color Coding:</span> Merah (pelanggaran), Hijau (prestasi), Biru (penanganan)</p>
                <p>• <span className="font-medium">Format Output:</span> File Excel (.xlsx) dengan format yang rapi dan mudah dibaca</p>
              </div>
            </div>
          </div>
        </div>
