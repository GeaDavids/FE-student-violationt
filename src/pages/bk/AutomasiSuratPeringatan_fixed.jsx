    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Nama Konfigurasi</label>
            <input
              type="text"
              value={formData.nama}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Judul Template</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Isi Template</label>
          <textarea
          ...
          <p className="mt-1 text-xs text-gray-500">
            Gunakan placeholder: {"{NAMA_SISWA}"}, {"{KELAS}"}, {"{TOTAL_SCORE}"}, {"{NISN}"}, {"{TANGGAL}"}, {"{NAMA_ORTU}"}
          </p>
        </div>
        ...
        <div className="flex items-center">
          ...
          />
          <label className="ml-2 block text-sm text-gray-900">Aktifkan konfigurasi</label>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 mb-8">
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Automasi Surat Peringatan</h1>
                <p className="text-gray-600 mt-1">Kelola konfigurasi dan pantau riwayat surat peringatan otomatis</p>
              </div>
              ...
              <div>
                {safeConfigs.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">Belum ada konfigurasi</h3>
                    <p className="mt-1 text-sm text-gray-500">Konfigurasi surat peringatan akan muncul di sini</p>
                  </div>
                ) : (
                        ...
                        <div className="p-6">
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <span className="text-2xl">{getSuratTypeIcon(config.jenisSurat)}</span>
                              <div>
                                <h3 className="text-lg font-semibold text-gray-900">{config.nama}</h3>
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getSuratTypeColor(config.jenisSurat)}`}>
                                  {config.jenisSurat}
                                </span>
                              </div>
                            </div>
                          ...
                          <div className="space-y-2 mb-4">
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-medium text-gray-900">{config.threshold} score</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="text-gray-500">Tingkat:</span>
                              <span className="font-medium text-gray-900">Level {config.tingkat}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="text-gray-500">Status:</span>
                              <span className={`font-medium ${config.isActive ? 'text-green-600' : 'text-gray-500'}`}>
              ...
              <div>
                {safeHistorySurat.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">Belum ada riwayat surat</h3>
                    <p className="mt-1 text-sm text-gray-500">Riwayat surat peringatan akan muncul di sini</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {safeHistorySurat.map((surat) => (
                      <div key={surat.id} className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow duration-200">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-lg">{getSuratTypeIcon(surat.jenisSurat)}</span>
                              <h4 className="font-semibold text-gray-900">{surat.judul}</h4>
                            </div>
                            <div className="grid grid-cols-2 gap-4 text-sm">
                              <div>
                                <span className="text-gray-500">Siswa:</span>
                                <p className="font-medium text-gray-900">{surat.student?.nama}</p>
                              </div>
                              <div>
                                <span className="text-gray-500">NISN:</span>
                                <p className="font-medium text-gray-900">{surat.student?.nisn}</p>
                              </div>
                              <div>
                                <span className="text-gray-500">Kelas:</span>
                                <p className="font-medium text-gray-900">{surat.student?.kelas}</p>
                              </div>
                              <div>
                                <span className="text-gray-500">Total Score:</span>
                                <p className="font-medium text-gray-900">{surat.totalScoreSaat}</p>
                              </div>
                              <div>
                                <span className="text-gray-500">Status:</span>
                                <p className={`font-medium ${surat.statusKirim === 'sent' ? 'text-green-600' : surat.statusKirim === 'pending' ? 'text-yellow-600' : 'text-red-600'}`}>
                                  {surat.statusKirim === 'sent' ? 'Terkirim' : surat.statusKirim === 'pending' ? 'Pending' : 'Gagal'}
                                </p>
                              </div>
                              <div>
                                <span className="text-gray-500">Tingkat:</span>
                                <p className="font-medium text-gray-900">Level {surat.tingkatSurat}</p>
                              </div>
                            </div>
                            <p className="text-xs text-gray-400 mt-2">
                              ...
                              {surat.tanggalKirim && (
                                <span className="ml-2 text-green-600">
                                  • Dikirim: {new Date(surat.tanggalKirim).toLocaleDateString('id-ID', {
                                    day: 'numeric',
                                    month: 'long',
                                    year: 'numeric'
                                  })}
                          ...
                          <div className="flex flex-col gap-2">
                            <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium border ${getSuratTypeColor(surat.jenisSurat)}`}>
                              {surat.jenisSurat}
                            </span>
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              surat.statusKirim === 'sent' ? 'bg-green-100 text-green-800' : 
                              surat.statusKirim === 'pending' ? 'bg-yellow-100 text-yellow-800' : 
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-semibold text-gray-900">Detail Konfigurasi Surat</h3>
                  <button
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-900 p-3 bg-gray-50 rounded-md">{selectedConfig.nama}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Jenis Surat</label>
                    <div className="p-3 bg-gray-50 rounded-md">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getSuratTypeColor(selectedConfig.jenisSurat)}`}>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900 p-3 bg-gray-50 rounded-md">{selectedConfig.threshold} score</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Tingkat</label>
                    <p className="text-sm text-gray-900 p-3 bg-gray-50 rounded-md">Level {selectedConfig.tingkat}</p>
                  </div>
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Judul Template</label>
                  <p className="text-sm text-gray-900 p-3 bg-gray-50 rounded-md">{selectedConfig.judulTemplate}</p>
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Isi Template</label>
                  <div className="text-sm text-gray-900 p-4 bg-gray-50 rounded-md max-h-60 overflow-y-auto">
                    <pre className="whitespace-pre-wrap font-sans">{selectedConfig.isiTemplate}</pre>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-md">
                    <div className={`w-3 h-3 rounded-full ${selectedConfig.isActive ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                    <span className={`text-sm font-medium ${selectedConfig.isActive ? 'text-green-600' : 'text-gray-500'}`}>
        ...
        {editingConfig && (
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-xl">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-semibold text-gray-900">Edit Konfigurasi</h3>
                  <button
