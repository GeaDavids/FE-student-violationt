...
const KelolaSiswa = () => {
  ...
  // Check if we're returning from detail page with selected class
  const { selectedClass, selectedClassName } = location.state || {};
  ...
  useEffect(() => {
    ...
    // Set selected class if returning from detail page
    if (selectedClass && selectedClassName) {
      setKelasDipilih(selectedClass);
      setNamaKelasDipilih(selectedClassName);
    }
            ...
            {kelasList.map((kelas) => (
              <button
                ...
                onClick={() => {
                  ...
                  setNamaKelasDipilih(kelas.namaKelas || kelas.name);
                }}
              ...
              >
                ...
                <div className="text-lg">{kelas.namaKelas || kelas.name}</div>
              </button>
            ))}
          </div>
        </div>
                ...
                {filteredSiswa.length > 0 ? (
                  filteredSiswa.map((siswa) => (
                    <tr key={siswa.id} className="hover:bg-gray-50">
                      ...
                      <td
                        className="border px-4 py-2 cursor-pointer hover:text-[#003366] hover:bg-blue-50 transition-colors"
                        onClick={() => {
