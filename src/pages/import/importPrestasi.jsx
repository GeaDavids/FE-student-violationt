// src/pages/import/ImportPrestasi.jsx
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import { FiUploadCloud } from "react-icons/fi";
import API from "../../api/api";

const ImportPrestasi = () => {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const res = await API.post("/import/prestasi", formData, {
        headers: {
      ...
      if (res.data.success) {
        setAlert({
          show: true,
          type: "success",
          message: `Berhasil mengimpor ${res.data.imported} prestasi!`,
        });
      } else {
        setAlert({
          show: true,
      ...
      setAlert({
        show: true,
        type: "error",
        message: "Gagal mengimpor data prestasi!",
      });
      console.error(err);
    } finally {
      setUploading(false);
    ...
    <div className="p-6">
      <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-200 max-w-xl mx-auto">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Unggah file Excel untuk mengimpor data prestasi. Pastikan format
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center gap-3 hover:border-blue-500 transition cursor-pointer">
          ...
          <input
            ...
            className="hidden"
            id="fileUploadPrestasi"
          />
          <label
            htmlFor="fileUploadPrestasi"
