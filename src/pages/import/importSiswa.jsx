...
const ImportSiswa = () => {
  ...
  React.useEffect(() => {
    ...
    // Fetch angkatan data
    API.get("/superadmin/masterdata/angkatan")
      .then((res) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      let response;
      const endpoints = ["/import/students"];
      ...
      let errorMsg =
        ...
        "Gagal mengimport data siswa. Pastikan endpoint import tersedia di backend.";
      // Jika ada error detail dari backend, tampilkan juga
      if (err.response?.data?.errorDetails) {
        errorMsg +=
          "\n" +
          (Array.isArray(err.response.data.errorDetails)
            ? err.response.data.errorDetails
                .map((e, i) =>
                  typeof e === "string"
                    ? e
                    : `Baris ${i + 1}: ${
                        e.error || e.reason || JSON.stringify(e)
                      }`
                )
                .join("\n")
            : JSON.stringify(err.response.data.errorDetails));
      }
          ...
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div>
              ...
              <p className="text-gray-600 text-xs mt-1">
                Upload file Excel (.xlsx) untuk menambahkan data siswa
              </p>
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-2">
                ...
                <code className="text-xs bg-white px-2 py-1 rounded border text-blue-900 block overflow-x-auto">
                  nisn | nama | gender | tempatLahir | tglLahir | alamat | noHp
                  | kelas | angkatan
                </code>
                <p className="text-xs text-blue-600 mt-1">
                  <strong>gender:</strong> L/P • <strong>tglLahir:</strong>{" "}
                  YYYY-MM-DD
                </p>
                <p className="text-xs text-red-600 mt-1">
                  <strong>Penting:</strong> Kolom <strong>NISN</strong>,{" "}
                  <strong>Tgl Lahir</strong>, <strong>No HP</strong>,{" "}
                  <strong>Angkatan</strong>, dan <strong>No HP Ortu</strong>{" "}
                  <u>wajib diubah ke format Text</u> di Excel agar data tidak
                  berubah otomatis.
                </p>
              </div>
            </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-xs text-amber-800">
                    File Excel siap untuk diimport. Pastikan format sesuai
                    template.
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-green-600 text-xs mt-1">
                      {result.message ||
                        `Berhasil mengimport ${result.imported || 0} siswa`}
                    </p>
                  </div>
                </div>
                      ...
                      <div className="space-y-1 max-h-16 overflow-y-auto">
                        ...
                        {result.errorDetails.length > 3 && (
                          <p className="text-xs text-yellow-600">
                            ...dan {result.errorDetails.length - 3} error
                            lainnya
                          </p>
                        )}
                      </div>
          ...
          <div className="mt-4 pt-3 border-t border-gray-200">
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    // Download template as XLSX (Excel) with text format for tglLahir, noHp, nohpOrtu
                    const headers = [
                      "nisn",
                      "nama",
                      "gender",
                    ...
                    // Set cell type to string for nisn, tglLahir, noHp, nohpOrtu (A2, E2, G2, K2)
                    ws["A2"].t = "s";
                    ws["E2"].t = "s";
                    ws["G2"].t = "s";
                    ws["K2"].t = "s";
                    // Set column width for better UX
                    ws["!cols"] = [
                      { wch: 12 }, // nisn
                      { wch: 18 }, // nama
                      { wch: 8 }, // gender
