...
const Sidebar = ({ onKelolaUserClick }) => {
  ...
  useEffect(() => {
    ...
    // Cek ke backend apakah user adalah wali kelas
    walikelasAPI.checkIsWaliKelas().then(setIsWaliKelas);
  }, []);
        ...
        <nav className="space-y-2">
                ...
                {showBKMonitoringSubmenu && (
                  <div className="ml-4 mt-2 space-y-1 border-l-2 border-cyan-400/30 pl-4">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/5 transition-all duration-200 group text-sm"
                    >
                      <FiSettings className="text-cyan-300 group-hover:text-white transition-colors text-sm" />
                      <span>Automasi Surat Peringatan</span>
                    </Link>
                  </div>
                )}
              </div>
          ...
          {(role === "guru" || role === "bk") && isWaliKelas && (
            <div className="space-y-2">
              ...
              >
                <FiUsers className="text-lg text-blue-200 group-hover:text-white transition-colors" />
                <span className="font-medium">Dashboard Wali Kelas</span>
              </Link>
            </div>
          )}
        </nav>
